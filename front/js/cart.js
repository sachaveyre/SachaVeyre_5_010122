...
//permet de verifier que nous sommes sur la page cart
let iscard = window.location.href.indexOf("cart");


//executer si nous sommes sur la page cart
if(iscard > -1) {
...
// Utilisation de la fonction postForm précedement créée
  ...
  function displayOrderId() {
    let ordrID = rdrID = localStorage.orderId;
    const orderIdElement = document.getElementById("orderId");

    console.log(orderIdElement);
    if(!localStorage.orderId ) {
    ordrID =  localStorage.getItem("orderId");
    }

    orderIdElement.textContent = ordrID;
  }
